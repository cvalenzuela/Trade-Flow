...
import { scale, soundScale } from './utils';
...
class Country {
  constructor(...args){
    ...
    this.pitch = soundScale(trade[this.Country][this.tradeType])
  ...
  Start(){
    for(let destinationName in this.trade){
      if(latlng[destinationName]){
        ...
        this.animationQue.push(setTimeout(()=> {this.system.addParticle(this.origin, destination, this.Country, destinationName, size, this.Color, this.Opacity, this.Trail, this.tradeType, this.showText, this.sound, this.pitch)}, (this.interval * 5000)/this.Speed));
